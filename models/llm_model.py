...
class LLMMassEstimator(BaseModel):
    ...
    def verify_mass_with_multimodal(self, image: np.ndarray, initial_estimation: dict, features: dict) -> dict:
        ...
        try:
            ...
            prompt = self._build_multimodal_prompt(initial_estimation)
    ...
    def _build_multimodal_prompt(self, initial_estimation: dict) -> str:
        initial_mass = initial_estimation.get('estimated_mass_g', 0)
        return f"""
음식 질량 추정 멀티모달 검증:

📊 초기 추정 정보:
- 초기 계산 질량: {initial_mass:.1f}g (참고용. 정확하지 않을 수 있음)

🔍 검증 과제:
이미지를 보고 다음 규칙에 따라 단계적으로 분석하세요.

1.  **1차 시각적 식별**:
    - 이미지에 보이는 물체를 있는 그대로 설명하세요
...
📋 응답 형식 (JSON):
{{
    "food_name": "<2단계에서 최종 결정된 가장 정확한 음식 이름>",
    "quoted_text": {{
        "product_name": "<라벨에서 읽은 제품명 그대로>",
        "weight": "<라벨에서 읽은 중량 그대로>"
    }},
    "verified_mass_g": <최종 검증된 질량(g)>,
    "confidence": <신뢰도(0.0~1.0)>,
    "reasoning": "<1, 2단계 분석 과정을 포함한 상세한 판단 근거>"
}}
"""
    ...
    def _process_parsed_response(self, parsed: dict, initial_estimation: dict) -> dict:
        ...
        reasoning = parsed.get('reasoning', '추론 과정 없음')
        ...
        if final_mass is None:
            final_mass = parsed.get('verified_mass_g', initial_estimation.get('estimated_mass_g', 0))
